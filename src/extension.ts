...
import { ConversionHoverProvider } from './hoverProvider';

function get_conversion_config(): caseConversion_config {
...
async function convert_singleFile(uri: vscode.Uri, converter: caseConverter): Promise<void> {
    try {
...
async function convert_folder(folder_uri: vscode.Uri, converter: caseConverter): Promise<void> {
    const config = vscode.workspace.getConfiguration('caseConverter');
    const supported_types: string[] = config.get('supportedFileTypes', ['js', 'ts', 'jsx', 'tsx', 'py']);
    const exclude_patterns: string[] = config.get('excludePatterns', ['node_modules', '.git', 'dist', 'build']);
...
async function findFilesInFolder(folder_path: string, supported_types: string[], exclude_patterns: string[]): Promise<string[]> {
    const files: string[] = [];

    async function scanDirectory(dir_path: string): Promise<void> {
...
export function activate(context: vscode.ExtensionContext) {
    ...
    const hoverProvider = new ConversionHoverProvider(converter);
    ...
    let decorationTimeout: NodeJS.Timeout | undefined; // Timer for auto-hiding decorations

    // Register command for converting selected text
    const convert_selection_command = vscode.commands.registerCommand(
        'caseConverter.convert_selection',
        () => {
    ...
    const convert_selection_withScopeCommand = vscode.commands.registerCommand(
        'caseConverter.convert_selection_withScope',
        () => {
            const editor = vscode.window.activeTextEditor;
            if (!editor) {
                vscode.window.showErrorMessage('No active editor found');
    ...
    const convert_document_command = vscode.commands.registerCommand(
        'caseConverter.convert_document',
        async () => {
            const editor = vscode.window.activeTextEditor;
            if (!editor) {
                vscode.window.showErrorMessage('No active editor found');
            ...
            const result = await vscode.window.showWarningMessage(
                'This will convert the entire document. Continue?',
                'Yes',
                'No'
            );
    ...
    const convert_current_fileCommand = vscode.commands.registerCommand(
        'caseConverter.convert_current_file',
        async () => {
            const editor = vscode.window.activeTextEditor;
            if (!editor) {
                vscode.window.showErrorMessage('No active editor found');
            ...
            const result = await vscode.window.showWarningMessage(
                'This will convert the entire file. Continue?',
                'Yes',
                'No'
            );
    ...
    const convert_fileCommand = vscode.commands.registerCommand(
        'caseConverter.convert_file',
        async (uri: vscode.Uri) => {
            ...
            const result = await vscode.window.showWarningMessage(
                `This will convert ${path.basename(uri.fsPath)}. Continue?`,
                'Yes',
                'No'
            );
    ...
    const convert_folder_command = vscode.commands.registerCommand(
        'caseConverter.convertFolder',
        async (uri: vscode.Uri) => {
            if (!uri) {
            ...
            const result = await vscode.window.showWarningMessage(
                `This will convert all supported files in ${path.basename(uri.fsPath)}. Continue?`,
                'Yes',
                'No'
            );
    ...
    const convert_current_file_toNewCaseCommand = vscode.commands.registerCommand(
        'caseConverter.convert_current_fileToNewCase',
        async () => {
            const editor = vscode.window.activeTextEditor;
            if (!editor) {
                vscode.window.showErrorMessage('No active editor found');
    ...
    const convert_folder_toNewCaseCommand = vscode.commands.registerCommand(
        'caseConverter.convert_folder_toNewCase',
        async (uri: vscode.Uri) => {
            if (!uri) {
                vscode.window.showErrorMessage('No folder selected');
                return;
    ...
    async function convertFileWithFormat(uri: vscode.Uri, format: string): Promise<void> {
        const config = vscode.workspace.getConfiguration('caseConverter');
        ...
        const result = await vscode.window.showWarningMessage(
            `This will convert ${path.basename(uri.fsPath)} to ${format}. Continue?`,
            'Yes',
            'No'
        );
    ...
    async function convertFolderWithFormat(uri: vscode.Uri, format: string): Promise<void> {
        const config = vscode.workspace.getConfiguration('caseConverter');
        const originalFormat = config.get('output_format');
        ...
        const result = await vscode.window.showWarningMessage(
            `This will convert all supported files in ${path.basename(uri.fsPath)} to ${format}. Continue?`,
            'Yes',
            'No'
        );
    ...
    const convertFileToLowercase = vscode.commands.registerCommand(
        'caseConverter.convertFileToLowercase',
        async (uri: vscode.Uri) => {
            if (!uri) {
                vscode.window.showErrorMessage('No file selected');
                return;
    ...
    const convertFileToUppercase = vscode.commands.registerCommand(
        'caseConverter.convertFileToUppercase',
        async (uri: vscode.Uri) => {
            if (!uri) {
                vscode.window.showErrorMessage('No file selected');
                return;
    ...
    const convertFileToEthanCase = vscode.commands.registerCommand(
        'caseConverter.convertFileToEthanCase',
        async (uri: vscode.Uri) => {
            if (!uri) {
                vscode.window.showErrorMessage('No file selected');
                return;
    ...
    const convertFolderToLowercase = vscode.commands.registerCommand(
        'caseConverter.convertFolderToLowercase',
        async (uri: vscode.Uri) => {
            if (!uri) {
                vscode.window.showErrorMessage('No folder selected');
                return;
    ...
    const convertFolderToUppercase = vscode.commands.registerCommand(
        'caseConverter.convertFolderToUppercase',
        async (uri: vscode.Uri) => {
            if (!uri) {
                vscode.window.showErrorMessage('No folder selected');
                return;
    ...
    const convertFolderToEthanCase = vscode.commands.registerCommand(
        'caseConverter.convertFolderToEthanCase',
        async (uri: vscode.Uri) => {
            if (!uri) {
                vscode.window.showErrorMessage('No folder selected');
                return;
    ...
    const toggleHighlighting_command = vscode.commands.registerCommand(
        'caseConverter.toggleHighlighting',
        () => {
    ...
    const convertWordAtCursor = vscode.commands.registerCommand('caseConverter.convertWordAtCursor', async (position?: vscode.Position) => {
        const editor = vscode.window.activeTextEditor;
        if (!editor) {
            return;
        }
        ...
        const conversion_config = get_conversion_config();
        conversion_config.output_format = favoriteCase as 'upperˉcase' | 'UPPERˉCASE' | 'ethan_case';
    ...
    const statusBarClickCommand = vscode.commands.registerCommand('caseConverter.statusBarClick', async () => {
        ...
        if (isDoubleClick) {
            ...
            const result = await vscode.window.showWarningMessage(
                `Convert entire file to ${favoriteCase}? (Double-click detected)`,
                'Yes',
                'No'
            );
    ...
    context.subscriptions.push(
        hoverDisposable,
        setFavoriteCaseCommand,
        convertWordAtCursor,
        convert_selection_command,
        convert_selection_withScopeCommand,
        convert_document_command,
        convert_current_fileCommand,
        convert_fileCommand,
        convert_folder_command,
        convert_current_file_toNewCaseCommand,
        convert_folder_toNewCaseCommand,
        convertFileToLowercase,
        convertFileToUppercase,
        convertFileToEthanCase,
        convertFolderToLowercase,
        convertFolderToUppercase,
        convertFolderToEthanCase,
        toggleHighlighting_command,
        showDecorationsTemporarily,
        toggleDecorations,
        statusBarClickCommand,
        onDidChangeActive_textEditor,
        onDidChange_text_document,
        onDidChangeSelection,
        onDidChangeActive_editor,
        onDidChange_document,
        status_barItem,
        decoration_provider
    );
}
